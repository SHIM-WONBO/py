...
while True:
    ...
    if not tmpbl: break
f.close()
del book_list[len(book_list)-1]
while True:
    bm = BM(book_list)
    print('\n====== 메뉴 번호를 입력하세요 ======')
    number = input("1. 도서 추가 \n2. 도서 검색 \n3. 도서 정보 수정 \n4. 도서 삭제 \n5. 현재 총 도서 목록 출력 \n6. 저장 \n7. 프로그램 나가기 \n입력 : ")
    if number == '1':
        book_list = copy.deepcopy(bm.menu1(book_list))
    elif number == '2':
        bm.menu2(book_list)
    elif number == '3':
        book_list = copy.deepcopy(bm.menu3(book_list))
    elif number == '4':
        book_list = copy.deepcopy(bm.menu4(book_list))
    elif number == '5':
        bm.menu5(book_list)
    elif number == '6':
        bm.menu6(book_list)
